...
public class JooqUpdateTest {
    ...
    void 업데이트_with_pojo() {
        ...
        // then
        Actor updateActor = actorRepository.findByActorId(actor.getActorId());
        assertThat(updateActor.getFirstName())
                .isEqualTo("Suri");
    }
    ...
    void 업데이트_일부_필드만() {
        ...
        var request = ActorUpdateRequest.builder().firstName("Suri").build();

        // when
        actorRepository.updateWithDto(newActorId, request);

        // then
        Actor updateActor = actorRepository.findByActorId(newActorId);
        assertThat(updateActor.getFirstName())
                .isEqualTo("Suri");
